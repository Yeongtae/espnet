...
from local import prepare_dataset_for_kaldiio
...
def main():
    ...
    stage1_dataset=['debug']
    basis_dataset = ['debug']
    stage2_dataset=['debug']
    recog_dataset=['debug']
...
## stage 0: Data preparation
    if stage <= 0:
        ...
        prepare_dataset_for_kaldiio.run('audio/projects/rnn-transducer-old/AIhub', 'audio/projects/rnn-transducer-old/AIhub/metadata.csv')
    else:
...
## stage 1: Feature Generation
    if stage <= 1:
        ...
        fbankdir='/audio/data_test/aihub/fbank'
        ...
        for x in stage1_dataset:
            ...
            cmd_nj = ' '+'--nj'+' '+'8'
            cmd_other = ' '+'--write_utt2num_frames true --write-utt2dur false'
            data_src = ' '+'data_test/'+x
            log = ' '+'exp/make_fbank/'+x
            # ret = subprocess.call([curr+'/espnet_utils/make_fbank.sh'+ \
            #         cmd_o+cmd_nj+cmd_other+data_src+log+' '+fbankdir], shell=True)
            ret = subprocess.call([curr+'/espnet_utils/make_fbank_myown.sh'+ \
                    cmd_o+cmd_nj+cmd_other+data_src+log+' '+fbankdir], shell=True)
            # ret = subprocess.call([curr + '/espnet_utils/make_fbank_myown.sh' + \
            #                        data_src ], shell=True)
            ...
            ret = subprocess.call([curr+'/utils/fix_data_dir.sh'+data_src], shell=True)

            ## Compute global CMVN(only train-clean)

            if x in basis_dataset:
                ret = subprocess.call(['compute-cmvn-stats'+' '+'scp:data_test/'+x+'/feats.scp'+data_src+'/cmvn.ark'], shell=True)
            ...
            dump_op = '--cmd'+' '+os.environ.get('train_cmd')+' '+'--nj 1'+' '+'--do_delta'+' '+do_delta+ \
                    ' '+'data_test/'+x+'/feats.scp'+' '+'data_test/'+'debug'+'/cmvn.ark'+' '+'exp/dump_feats/'+x+' '+dump_dir
    ...
    dict_path = 'data_test/lang_1char/'+traindata+'_units.txt'
...
## stage 2: Dictionary and Json Data Preparation
    if stage <= 2:
        ...
        for x in stage2_dataset:
            ## make dictionary
            os.makedirs(os.getcwd()+'/data_test/lang_1char', exist_ok=True)
            curr = os.getcwd()
            opt = 'cut -f 2- -d" "'+' '+'|'+' '+'tr " " "\\n"'+' '+'|'+' '+'sort | LC_COLLATE="ko_KR.UTF-8" uniq | '+"grep -v -e '^\\s*$'"
                   # "awk '{{print $0 " " NR+1}}'"+' >> '+os.getcwd()+'/data/lang_1char/'+x+'_units.txt'
            cmd = 'python'+' '+curr+'/../../../utils/'+'text2token.py'+' '+'-s 1 -n 1 data_test/'+x+'/text'
            ...
            if x in basis_dataset:
                ...
                with open(os.getcwd()+'/data_test/lang_1char/'+x+'_units.txt', 'w+') as dicfile:
...
## stage 3: encoder training or transfer
    if stage <= 3:
        ...
        enc_cmd = os.environ.get('cuda_cmd')+' '+'--gpu '+ngpu+' '+expdir+'/traing.log '
        enc_option = 'asr_train.py '+ \
                '--config '+ train_config+' '+ \
                '--preprocess-conf '+preprocess_config+' '+ \
                '--ngpu '+ngpu+' '+ \
                '--backend '+backend+' '+ \
        ...
        subprocess.call([enc_cmd+enc_option], shell=True)
    else:
